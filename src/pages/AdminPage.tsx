...
import { db, Product, Promotion, handleFirestoreError, OperationType } from '../firebase';
    ...
    <div className="flex-1 p-4 md:p-8 overflow-y-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex bg-surface rounded-lg p-1 border border-border">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'inventory' ? 'bg-primary text-white' : 'text-ink-muted hover:text-ink'}`}
          >
            <Package className="w-4 h-4" /> Inventario
          </button>
          <button
            onClick={() => setActiveTab('promotions')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'promotions' ? 'bg-primary text-white' : 'text-ink-muted hover:text-ink'}`}
          >
            <Tag className="w-4 h-4" /> Promociones
          </button>
          <button
            onClick={() => setActiveTab('inflation')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'inflation' ? 'bg-primary text-white' : 'text-ink-muted hover:text-ink'}`}
          >
      ...
      {activeTab === 'inventory' && (
        <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 border-b border-border flex justify-end">
            <button onClick={handleAddDemoProduct} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors">
                    ...
                    {editingId === product.id ? (
                      <>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button onClick={() => handleSave(product.id)} className="text-primary hover:text-blue-700 bg-accent p-2 rounded-lg inline-flex items-center transition-colors">
                ...
                {products.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-6 py-8 text-center text-ink-muted">
                      No hay productos. Usa "Importar Catálogo" para crear la base de datos de productos.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {activeTab === 'promotions' && (
        <div className="bg-surface rounded-xl border border-border shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={handleCreateDemoPromotion} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors">
          ...
          <div className="grid gap-4">
            {promotions.map(promo => (
              <div key={promo.id} className="p-4 border border-border rounded-lg flex justify-between items-center bg-bg">
                ...
                <button onClick={() => handleDeletePromotion(promo.id)} className="text-red-500 hover:text-red-700 p-2 border border-red-100 hover:bg-red-50 rounded-lg transition-colors">
                  <Trash2 className="w-5 h-5" />
                </button>
              </div>
            ))}
            {promotions.length === 0 && (
              <p className="text-ink-muted flex items-center justify-center p-8 border border-dashed border-border rounded-lg">No hay promociones configuradas</p>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'inflation' && (
        <div className="bg-surface rounded-xl border border-border shadow-sm p-6 max-w-md">
          ...
          <p className="text-sm text-ink-muted mb-6">Ajusta el precio de venta de TODOS los productos en un porcentaje específico. Ejemplo: 5 para aumentar 5%.</p>
